...
const count = (target, input) => {
  ...
  if (input.length === 0) {
    count = input.filter(a => (target === a)).length;
  } else {
    count = input.reduce((a, b) => a.concat(b)).filter(a => (target === a)).length;
  }

  return count;
  // return input.reduce((a, b) => a.concat(b)).filter(a => (target === a)).length;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((arr1, arr2) => arr1.concat(arr2)).reduce((a, b) => a + b, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arrayInput => arrayInput.filter((array => (Number.isInteger(array) && array % 5 === 0)))).map(arr => arr.map(a => Math.pow(2, a)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(datum => (datum.gender === 'male' || datum.gender === 'female')).map(d => d.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((acc, cur) => parseInt(cur.height) > parseInt(acc.height) ? acc : cur);
