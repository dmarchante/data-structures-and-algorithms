...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(a => (a.mass > parseInt((arr.filter((a) => (a.name === 'Luke Skywalker'))[0].mass)))).map(name => name.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  return arr.sort((a, b) => (typeof(a[property]) === 'string' || typeof(b[property]) === 'string') ? a[property].localeCompare(b[property]) : a[property] - b[property]);
};
