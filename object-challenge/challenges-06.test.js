...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  Object.values(arr).forEach(a => houses.push(a.house));
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).forEach(a => {
    if (a.name === character) {
      (a.children.length) ? hasChildren = true : hasChildren = false;
    }
  });
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.entries(arr).forEach(a => {
    if (a[1].name === character) {
      (a[1].children.length) ? hasChildren = true : hasChildren = false;
    }
  });
...
const houseSurvivors = (arr) => {
  ...
  Object.values(arr).forEach(a => {
    ...
    if(a.spouse && !deceasedSpouses.includes(a.spouse)) {
      sizeOfHouse++;
    }
